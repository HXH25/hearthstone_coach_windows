...
if (-not (Test-Path .\V0_5_0_6_CARD_ART_GUIDE_FIX.md)) { throw "missing V0_5_0_6_CARD_ART_GUIDE_FIX.md" }
...
Write-Host "`n== card art / guide sanity ==" -ForegroundColor Cyan
$overlay = Get-Content .\src\demo\overlay.rs -Raw
foreach ($needle in @('CardPortraits', 'CardTiles', 'strip_png_iccp_chunk', 'load_from_memory_with_format')) {
    if ($overlay -notlike "*$needle*") { throw "overlay missing robust card art behavior: $needle" }
}
$server = Get-Content .\src\demo\server.rs -Raw
if ($server -notlike '*auto_prepare_guide*') { throw 'server missing auto_prepare_guide flow' }
$example = Get-Content .\hearthcoach_demo.example.json -Raw | ConvertFrom-Json
if ($null -eq $example.agent.auto_prepare_guide) { throw 'example config missing agent.auto_prepare_guide' }

Write-Host "`nV0.5.0.6 verification passed." -ForegroundColor Green
