...
foreach ($needle in @('win.rustup.rs', 'vs_BuildTools.exe', 'cargo build --release --bin hearthcoach', 'target\release\hearthcoach.exe', 'HearthSim.HearthstoneDeckTracker', 'HDT-Installer.exe', 'HEARTHCOACH_HEARTHDB_DLL')) {
    if ($launcher -notlike "*$needle*") { throw "launcher missing expected bootstrap step: $needle" }
}

Write-Host "`nV0.5.0.3 verification passed." -ForegroundColor Green
